...
- Existe además un benchmark supervisado por regresión logística multinomial sobre `Sleep-EDF ST` que supera claramente a la heurística manual; la mejor versión probada usa `spectral_plus_hjorth` (núcleo espectral + `hjorthMobility` + `hjorthComplexity`) y puede usarse ya como fuente experimental del `Hyp`, manteniendo la heurística manual como fallback y referencia.
