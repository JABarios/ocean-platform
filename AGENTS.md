...
En `kappa`, además existen tests sintéticos compartidos para validar el staging heurístico por `FMD`, la distinción entre sigma continua y husos discretos, la separación `N2/N3` mediante `slowWaveFraction`, y una noche sintética larga reestadiada de extremo a extremo, incluyendo épocas con artefactos inyectados a propósito. También existe validación real sobre `Sleep-EDF ST` tanto para la heurística manual como para un benchmark supervisado por **regresión logística multinomial** (`test_sleep_edf_logreg`), cuya mejor variante medida actualmente es `spectral_plus_hjorth_plus_perm_spindlecount` sobre `15` sujetos `Sleep-EDF ST`.
...
- Existe además un benchmark supervisado por regresión logística multinomial sobre `Sleep-EDF ST` que supera claramente a la heurística manual; la mejor versión probada en benchmark usa `spectral_plus_hjorth_plus_perm_spindlecount` (núcleo espectral + `hjorthMobility` + `hjorthComplexity` + `permutationEntropy` + `spindleEventCount`) y alcanza aproximadamente `70.17%` de `accuracy` con `kappa ≈ 0.596` sobre `15` sujetos. La integración embebida del visor/WASM puede evolucionar a esa variante, manteniendo la heurística manual como fallback y referencia.
